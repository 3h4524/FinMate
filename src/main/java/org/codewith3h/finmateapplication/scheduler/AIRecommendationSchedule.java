    ...
    public void scheduleBudgetRecommendation() throws JOSEException, MessagingException {
        ...
        for(User user : premiumUser) {
            if(featureService.userHasFeature(user.getId(), FeatureCode.SMART_REMINDER.name())){
                ...
                if(budgetPredictionResponse != null && budgetPredictionResponse.getBudgets() != null){
                    emailService.sendBudgetRecommendation(user.getId(), budgetPredictionResponse.getBudgets());
                    log.info("Budget recommendation sent to user: {}", user.getId());
                } else {
                    log.warn("No budget predictions available for user: {}", user.getId());
                }
            }
        }
    }
