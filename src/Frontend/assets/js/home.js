...
async function initializePage() {
    try {
        ...
        // Load sidebar.html and profile-sidebar.html concurrently
        let profileSidebarElement = null;
        await Promise.all([
            fetch('sidebar.html').then(response => response.text()),
            fetch('profile-sidebar.html').then(response => response.text())
        ]).then(([sidebarHtml, profileSidebarHtml]) => {
            // Insert sidebar HTML
            ...
            // Insert profile sidebar HTML
            document.getElementById('profile-sidebar-container').innerHTML = profileSidebarHtml;
            console.log('Profile sidebar HTML inserted.');
            profileSidebarElement = document.getElementById('profileSidebar');
            if (profileSidebarElement) {
                console.log('Profile sidebar element found after insertion.');
            } else {
                console.error('Profile sidebar element NOT found after insertion.');
            }

            // Attach event listener for the close button on profile sidebar
            ...
            if (viewProfileBtn) {
                console.log('View profile button found. Attaching click listener.');
                viewProfileBtn.addEventListener('click', () => {
                    if (profileSidebarElement) {
                        console.log('View profile clicked. Current active state:', profileSidebarElement.classList.contains('active'));
                        if (profileSidebarElement.classList.contains('active')) {
                            profileSidebarElement.classList.remove('active');
                            console.log('Removed active class. Sidebar should hide.');
                        } else {
                            profileSidebarElement.classList.add('active');
                            console.log('Added active class. Sidebar should show.');
                        ...
                        console.error("Profile sidebar element not found when clicking view profile (inside click handler).");
                    }
                });
            } else {
                console.error("View profile button (viewProfileBtn) NOT found after sidebar HTML inserted.");
            }
        });
